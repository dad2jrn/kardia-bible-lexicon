...
export interface UseGeneratorReturn {
  ...
  isBusy: boolean
  canGenerate: boolean
  generateFresh: (category: CategorySelection, model: ModelId, apiKey: string) => Promise<void>
  ...
  switch (step) {
    case 'generating':
      return {
        ...
        helperText: 'Running Anthropic primary pass…',
        tone: 'info',
      }
    case 'validating':
      return {
...
export function useGenerator(): UseGeneratorReturn {
  ...
  const runPipeline = useCallback(
    async (params: GeneratorParams) => {
      ...
      const prompt = buildGenerationPrompt(params.category.label)

      try {
        const generated = await runGeneration(params.apiKey, prompt, params.model, {
          signal: controller.signal,
        })
        setEntry(generated)

        setStep('validating')
        const validated = await runValidation(params.apiKey, generated, params.model, {
          signal: controller.signal,
        })
        ...
        try {
          verses = await runKardiaVerseTranslation(params.apiKey, generated, params.model, {
            signal: controller.signal,
          })
  ...
  const generateFresh = useCallback(
    async (category: CategorySelection, model: ModelId, apiKey: string) => {
      await runPipeline({ category, model, apiKey })
    },
    [runPipeline],
  )
